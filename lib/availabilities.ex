defmodule Availabilities do
  ...
  def build(%Params{schedules: schedules, settings: slots}, date), do:
    date
    |> Timex.weekday()
    |> num_to_weekday()
  ...
  @spec select_store_open_days({:ok, Timex.weekday_name} | {:error, :invalid_date}, [Schedule.t]) :: {:ok, [Schedule.t]} | {:error, :invalid_date}
  defp select_store_open_days({:ok, weekday}, schedules), do:
    {:ok, Enum.filter(schedules, &store_open?(weekday, &1))}
